...
int main()
{
 ...
 //FIXMED -bonus - 10 bonus points - add loop until user wants to quit
    ...
    // the loop will execute the following block of code
    {
        ...
        cin >> ch >> x2 >> ch >> y2 >> ch; //FIXMED - Read/parse the second point and store data into variables x2 and y2
        printf("(x2, y2) = (%d, %d)\n", x2, y2);
...
void unitTest()
{
    ...
    result =  findDistance(0, 1, 1, 1);//accept the result if it's less than the error of margin
    expected = 1.00f;
    assert( fabs(result - expected) <= epsilon);

     result =  findDistance(2, 2, 4, 2); 
     expected = 2.00f;
    assert( fabs(result - expected) <= epsilon);
